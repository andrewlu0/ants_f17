    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if self.ant.container == True and self.ant.ant is None:
                    self.ant.contain_ant(insect)
                elif insect.container == True and self.ant.container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while near == None and check.name!='Hive' and dist <= self.max_range:
            if len(check.bees)==0 or dist < self.min_range:
        ...
        if near == None:
            ...
            return random_or_none(near.bees)
        # END Problem 5
...
class ScubaThrower(ThrowerAnt):
    "ScubaAnt can be placed on water"
    name = 'Scuba'
    implemented = True
    watersafe = True
    food_cost = 6
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while place.exit is not None and place.exit.name != 'Hive':
            if isinstance(place.ant,QueenAnt):
                self.reduce_armor(armor)
            if place.ant is not None and place.ant not in self.buffedAnts:
                place.ant.damage *= 2
                self.buffedAnts.append(place.ant)
            place = place.exit
